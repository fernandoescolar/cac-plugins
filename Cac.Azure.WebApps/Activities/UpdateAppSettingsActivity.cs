...
namespace Cac.Azure.WebApps.Activities
{
    ...
    public class UpdateAppSettingsActivity : CacActivity<UpdateAppSettingsModel>
    {
        protected override Task<IEnumerable<ICacCommand>> OnPlanAsync(UpdateAppSettingsModel model, IExecutionContext context)
        {
            var settings = model.Settings ?? GetSettings(model.SettingsFilepath) ?? new Dictionary<string, string>();
            var command = UpdateAppSettingsCommandFactory.CreateCommand(model.Webapp, model.ServicePrincipal, settings, model.IncludeDeletion);
